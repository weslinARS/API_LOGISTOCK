import { HttpStatus, Inject, Injectable } from "@nestjs/common";
import { ApiResponse, CustomError } from "src/shared/class/api-response.class";
import { PRODUCT_BRAND_REPOSITORY_SYMBOL } from "src/shared/common/common-constant";
import { IProductBrandRepository } from "src/shared/interface/repositories/product-brand-repository.interface";

@Injectable()
export class ProductBrandsService {
	constructor(
		@Inject(PRODUCT_BRAND_REPOSITORY_SYMBOL)
		private readonly productBrandRepository: IProductBrandRepository,
	) {}

	async create(data: { name: string }) {
		try {
			const exists =
				await this.productBrandRepository.verifyIfExistsByName(
					data.name,
				);
			if (exists)
				throw new CustomError({
					errorCode: "RECORD_ALREADY_EXISTS",
					message: "La marca del producto ya existe",
					statusCode: HttpStatus.CONFLICT,
				});
			const record = await this.productBrandRepository.create({
				name: data.name,
			});
			if (!record)
				throw new CustomError({
					errorCode: "QUERY_ERROR",
					message: "No se pudo crear la marca del producto",
					statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
				});
			return new ApiResponse({
				message: "Marca del producto creada exitosamente",
				statusCode: HttpStatus.CREATED,
			});
		} catch (error) {
			if (error instanceof CustomError) throw error.toHttpException();
			throw error;
		}
	}
}
